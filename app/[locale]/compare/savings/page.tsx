...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildMetadata({
    ...
    locale: localeTyped,
    title: `${t("title")} - Savings Insurance Pairs`,
    description: `${t("description")} Browse Savings insurance product comparisons by company, region, currency, premium term, IRR, and break-even year on Policy Vector.`,
    robots: { index: false, follow: true },
  });
}

export default async function CompareSavingsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  ...
  // Generate all unique pairs (C(n, 2)) for the same-category browse page
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="flex items-end gap-3 mb-2">
        <h1 className="text-3xl font-bold text-gray-900">{"储蓄险对比"}</h1>
        <p className="text-xs text-gray-400 pb-1">{`共 ${vectors.length} 款储蓄险 · ${pairs.length} 组对比`}</p>
      </div>
