      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {comparisons.map((comp) => (
          ...
          >
            ...
            <p className="font-semibold text-gray-900 text-sm mb-1">{comp.productA.displayName}</p>
            <p className="text-xs text-gray-500 mb-2">vs</p>
            <p className="font-semibold text-gray-900 text-sm mb-2">{comp.productB.displayName}</p>
            <div className="flex justify-between text-xs text-gray-400">
              <span>{comp.productA.company.displayName}</span>
              <span>{comp.productB.company.displayName}</span>
            </div>
          </Link>
        ))}
      </div>
