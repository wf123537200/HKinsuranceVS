...
export default function CompareAIButton({ comparisonId, productAId, productBId }: Props) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Reload to show the generated content
  ...
  if (!session) {
    return (
      <div className="text-center py-4">
        <p className="text-sm text-gray-600 mb-3">Sign in to generate AI comparisons (3 free per day).</p>
        <Link
          href="/login"
  ...
  return (
    <div>
      ...
      {result && <p className="mt-2 text-sm text-green-600">Comparison generated successfully!</p>}
    </div>
  );
}
